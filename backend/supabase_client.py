...
# def upload_file_and_get_url(local_path: str, dest_filename: str):
#     """Uploads file to Supabase storage and returns public URL (or path)."""
#     try:
#         with open(local_path, "rb") as f:
#             resp = sb.storage.from_(SUPABASE_BUCKET).upload(dest_filename, f)
#         # make public - depends on bucket policies
#         url = sb.storage.from_(SUPABASE_BUCKET).get_public_url(dest_filename)
#         return url.get('publicURL') or url
#     except Exception as e:
#         logger.exception("Supabase file upload failed")
#         raise

# def insert_document_record(doc_type: str, raw_text: str, structured_json: dict, file_url: str):
#     """Insert into `documents` table. Uses Supabase client to insert into Postgres.
#     Assumes `documents` table schema exists as in README.
#     """
#     try:
#         payload = {
#             "id": str(uuid.uuid4()),
...
#         }
#         resp = sb.table("documents").insert(payload).execute()
#         return resp
#     except Exception:
